...
const Dashboard = () => {
  ...
  useEffect(() => {
    async function getTotalNumberOfDonations() {
      const res = await fetch(
        `https://api.divinedonors/order/donations/`
      );
    ...
    async function getAllDonations(totalNumberOfElements) {
      const res = await fetch(
        `https://api.divinedonors/order/donations?size=${totalNumberOfElements}`
      );
    ...
    async function init() {
      try {
        ...
        const sum = donationsData.content
          .map((d) => ({
            ...d,
            placeUrl: `https://divinedonors/fundraiser/${d.placeId}`,
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 450,
      renderCell: (params) => (
        <Link href={`https://divinedonors/fundraiser/${params.value}`}>{`https://divinedonors/fundraiser/${params.value}`}</Link>
      ),
    },
